...
Func SaveConfigsBis()
	...
	IniWrite(@ScriptDir& "\settings.ini", "Account info", "pass", $PassD3)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "Unidentified", $Unidentified)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "monsterList", $monsterList)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "specialmonsterList", $specialmonsterList)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SequenceFile", $SequenceFile)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "QualiteItem", $QualiteItem)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "Key1", $key1)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "Key2", $key2)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "Key3", $key3)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "Key4", $key4)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "PreBuff1", $PreBuff1)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "ToucheBuff1", $ToucheBuff1)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "delaiBuff1", $delaiBuff1)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "PreBuff2", $PreBuff2)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "ToucheBuff2", $ToucheBuff2)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "delaiBuff2", $delaiBuff2)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "PreBuff3", $PreBuff3)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "ToucheBuff3", $ToucheBuff3)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "delaiBuff3", $delaiBuff3)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "PreBuff4", $PreBuff4)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "ToucheBuff4", $ToucheBuff4)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "delaiBuff4", $delaiBuff4)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellOnLeft", $SpellOnLeft)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellDelayLeft", $SpellDelayLeft)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellTypeLeft", $SpellTypeLeft)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellEnergyNeedsLeft", $SpellEnergyNeedsLeft)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellLifeLeft", $SpellLifeLeft)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellDistanceLeft", $SpellDistanceLeft)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellOnRight", $SpellOnRight)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellDelayRight", $SpellDelayRight)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellTypeRight", $SpellTypeRight)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellEnergyNeedsRight", $SpellEnergyNeedsRight)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellLifeRight", $SpellLifeRight)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellDistanceRight", $SpellDistanceRight)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellOn1", $SpellOn1)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellDelay1", $SpellDelay1)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellType1", $SpellType1)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellEnergyNeeds1", $SpellEnergyNeeds1)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellLife1", $SpellLife1)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellDistance1", $SpellDistance1)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellOn2", $SpellOn2)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellDelay2", $SpellDelay2)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellType2", $SpellType2)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellEnergyNeeds2", $SpellEnergyNeeds2)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellLife2", $SpellLife2)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellDistance2", $SpellDistance2)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellOn3", $SpellOn3)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellDelay3", $SpellDelay3)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellType3", $SpellType3)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellEnergyNeeds3", $SpellEnergyNeeds3)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellLife3", $SpellLife3)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellDistance3", $SpellDistance3)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellOn4", $SpellOn4)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellDelay4", $SpellDelay4)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellType4", $SpellType4)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellEnergyNeeds4", $SpellEnergyNeeds4)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellLife4", $SpellLife4)
	IniWrite(@ScriptDir& "\settings.ini", "Run info", "SpellDistance4", $SpellDistance4)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","LifeForPotion",$LifeForPotion)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","PotionStock",$PotionStock)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","TakeShrines",$TakeShrines)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","repairafterxxgames",$repairafterxxgames)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","maxgamelength",$maxgamelength)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","attackRange",$attackRange)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","grabRange",$grabRange)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","MonsterTri",$MonsterTri)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","MonsterRefresh",$MonsterRefresh)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","ItemRefresh",$ItemRefresh)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","MonsterPriority",$MonsterPriority)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","InventoryCheck",$InventoryCheck)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","attacktimeout",$attacktimeout)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","grabtimeout",$grabtimeout)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","Gestion_affixe",$Gestionaffixe)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","Gestion_affixe_loot",$Gestionaffixeloot)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","BanAffixList",$BanAffixList)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","Gest_affixe_ByClass",$GestaffixeByClass)
    iniwrite(@ScriptDir& "\settings.ini", "Run info","Life_Arcane",$LifeArcane)
    iniwrite(@ScriptDir& "\settings.ini", "Run info","Life_Peste",$LifePeste)
    iniwrite(@ScriptDir& "\settings.ini", "Run info","Life_Profa",$LifeProfa)
    iniwrite(@ScriptDir& "\settings.ini", "Run info","Life_Mine",$LifeMine)
    iniwrite(@ScriptDir& "\settings.ini", "Run info","Life_Spore",$LifeSpore)
    iniwrite(@ScriptDir& "\settings.ini", "Run info","Life_Arm",$LifeArm)
    iniwrite(@ScriptDir& "\settings.ini", "Run info","Life_Lave",$LifeLave)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","Life_Proj",$LifeProj)
    iniwrite(@ScriptDir& "\settings.ini", "Run info","Life_Ice",$LifeIce)
    iniwrite(@ScriptDir& "\settings.ini", "Run info","Life_Poison",$LifePoison)
    iniwrite(@ScriptDir& "\settings.ini", "Run info","Life_Explo",$LifeExplo)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","ResLife",$ResLife)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","ResActivated",$ResActivated)
	iniwrite(@ScriptDir& "\settings.ini", "Run info","UsePath",$UsePath)

	AjoutLog("Enregistrement des modifs du fichier settings.ini")

EndFunc;==>SaveConfigsBis

Func LoadConfigsBis()
	$PassD3 = IniRead(@ScriptDir& "\settings.ini", "Account info", "pass", "")
	$Unidentified = IniRead(@ScriptDir& "\settings.ini", "Run info", "Unidentified", "")
	$monsterList = IniRead(@ScriptDir& "\settings.ini", "Run info", "monsterList", "")
	$specialmonsterList = IniRead(@ScriptDir& "\settings.ini", "Run info", "specialmonsterList", "")
	$SequenceFile = IniRead(@ScriptDir& "\settings.ini", "Run info", "SequenceFile", "")
	$QualiteItem = IniRead(@ScriptDir& "\settings.ini", "Run info", "QualiteItem", "")
	$key1 = IniRead(@ScriptDir& "\settings.ini", "Run info", "key1", "")
	$key2 = IniRead(@ScriptDir& "\settings.ini", "Run info", "key2", "")
	$key3 = IniRead(@ScriptDir& "\settings.ini", "Run info", "key3", "")
	$key4 = IniRead(@ScriptDir& "\settings.ini", "Run info", "key4", "")
	$PreBuff1 = IniRead(@ScriptDir& "\settings.ini", "Run info", "PreBuff1", "")
	$ToucheBuff1 = IniRead(@ScriptDir& "\settings.ini", "Run info", "ToucheBuff1", "")
	$delaiBuff1 = IniRead(@ScriptDir& "\settings.ini", "Run info", "delaiBuff1", "")
	$PreBuff2 = IniRead(@ScriptDir& "\settings.ini", "Run info", "PreBuff2", "")
	$ToucheBuff2 = IniRead(@ScriptDir& "\settings.ini", "Run info", "ToucheBuff2", "")
	$delaiBuff2 = IniRead(@ScriptDir& "\settings.ini", "Run info", "delaiBuff2", "")
	$PreBuff3 = IniRead(@ScriptDir& "\settings.ini", "Run info", "PreBuff3", "")
	$ToucheBuff3 = IniRead(@ScriptDir& "\settings.ini", "Run info", "ToucheBuff3", "")
	$delaiBuff3 = IniRead(@ScriptDir& "\settings.ini", "Run info", "delaiBuff3", "")
	$PreBuff4 = IniRead(@ScriptDir& "\settings.ini", "Run info", "PreBuff4", "")
	$ToucheBuff4 = IniRead(@ScriptDir& "\settings.ini", "Run info", "ToucheBuff4", "")
	$delaiBuff4 = IniRead(@ScriptDir& "\settings.ini", "Run info", "delaiBuff4", "")
	$SpellOnLeft = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellOnLeft", "")
	$SpellDelayLeft = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellDelayLeft", "")
	$SpellTypeLeft = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellTypeLeft", "")
	$SpellEnergyNeedsLeft = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellEnergyNeedsLeft", "")
	$SpellLifeLeft = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellLifeLeft", "")
	$SpellDistanceLeft = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellDistanceLeft", "")
	$SpellOnRight = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellOnRight", "")
	$SpellDelayRight = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellDelayRight", "")
	$SpellTypeRight = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellTypeRight", "")
	$SpellEnergyNeedsRight = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellEnergyNeedsRight", "")
	$SpellLifeRight = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellLifeRight", "")
	$SpellDistanceRight = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellDistanceRight", "")
	$SpellOn1 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellOn1", "")
	$SpellDelay1 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellDelay1", "")
	$SpellType1 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellType1", "")
	$SpellEnergyNeeds1 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellEnergyNeeds1", "")
	$SpellLife1 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellLife1", "")
	$SpellDistance1 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellDistance1", "")
	$SpellOn2 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellOn2", "")
	$SpellDelay2 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellDelay2", "")
	$SpellType2 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellType2", "")
	$SpellEnergyNeeds2 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellEnergyNeeds2", "")
	$SpellLife2 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellLife2", "")
	$SpellDistance2 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellDistance2", "")
	$SpellOn3 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellOn3", "")
	$SpellDelay3 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellDelay3", "")
	$SpellType3 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellType3", "")
	$SpellEnergyNeeds3 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellEnergyNeeds3", "")
	$SpellLife3 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellLife3", "")
	$SpellDistance3 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellDistance3", "")
	$SpellOn4 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellOn4", "")
	$SpellDelay4 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellDelay4", "")
	$SpellType4 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellType4", "")
	$SpellEnergyNeeds4 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellEnergyNeeds4", "")
	$SpellLife4 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellLife4", "")
	$SpellDistance4 = IniRead(@ScriptDir& "\settings.ini", "Run info", "SpellDistance4", "")
	$LifeForPotion = IniRead(@ScriptDir& "\settings.ini", "Run info", "LifeForPotion", "")
	$PotionStock = IniRead(@ScriptDir& "\settings.ini", "Run info", "PotionStock", "")
	$TakeShrines = IniRead(@ScriptDir& "\settings.ini", "Run info", "TakeShrines", "")
	$repairafterxxgames = IniRead(@ScriptDir& "\settings.ini", "Run info", "repairafterxxgames", "")
	$maxgamelength = IniRead(@ScriptDir& "\settings.ini", "Run info", "maxgamelength", "")
	$attackRange = IniRead(@ScriptDir& "\settings.ini", "Run info", "attackRange", "")
	$grabRange = IniRead(@ScriptDir& "\settings.ini", "Run info", "grabRange", "")
	$MonsterTri = IniRead(@ScriptDir& "\settings.ini", "Run info", "MonsterTri", "")
	$MonsterRefresh = IniRead(@ScriptDir& "\settings.ini", "Run info", "MonsterRefresh", "")
	$ItemRefresh = IniRead(@ScriptDir& "\settings.ini", "Run info", "ItemRefresh", "")
	$MonsterPriority = IniRead(@ScriptDir& "\settings.ini", "Run info", "MonsterPriority", "")
	$InventoryCheck = IniRead(@ScriptDir& "\settings.ini", "Run info", "InventoryCheck", "")
	$attacktimeout = IniRead(@ScriptDir& "\settings.ini", "Run info", "attacktimeout", "")
	$grabtimeout = IniRead(@ScriptDir& "\settings.ini", "Run info", "grabtimeout", "")
	$Gestionaffixe = IniRead(@ScriptDir& "\settings.ini", "Run info", "Gestion_affixe", "")
	$Gestionaffixeloot = IniRead(@ScriptDir& "\settings.ini", "Run info", "Gestion_affixe_loot", "")
	$BanAffixList = IniRead(@ScriptDir& "\settings.ini", "Run info", "BanAffixList", "")
	$GestaffixeByClass = IniRead(@ScriptDir& "\settings.ini", "Run info", "Gest_affixe_ByClass", "")
	$LifeArcane = IniRead(@ScriptDir& "\settings.ini", "Run info", "Life_Arcane", "")
	$LifeProj = IniRead(@ScriptDir& "\settings.ini", "Run info", "Life_Proj", "")
	$LifePeste = IniRead(@ScriptDir& "\settings.ini", "Run info", "Life_Peste", "")
	$LifeProfa = IniRead(@ScriptDir& "\settings.ini", "Run info", "Life_Profa", "")
	$LifeIce = IniRead(@ScriptDir& "\settings.ini", "Run info", "Life_Ice", "")
	$LifePoison = IniRead(@ScriptDir& "\settings.ini", "Run info", "Life_Poison", "")
	$LifeExplo = IniRead(@ScriptDir& "\settings.ini", "Run info", "Life_Explo", "")
	$LifeLave = IniRead(@ScriptDir& "\settings.ini", "Run info", "Life_Lave", "")
	$LifeMine = IniRead(@ScriptDir& "\settings.ini", "Run info", "Life_Mine", "")
	$LifeArm = IniRead(@ScriptDir& "\settings.ini", "Run info", "Life_Arm", "")
	$LifeSpore = IniRead(@ScriptDir& "\settings.ini", "Run info", "Life_Spore", "")
	$UsePath = IniRead(@ScriptDir& "\settings.ini", "Run info", "UsePath", "")
	$ResActivated = IniRead(@ScriptDir& "\settings.ini", "Run info", "ResActivated", "")
	$ResLife = IniRead(@ScriptDir& "\settings.ini", "Run info", "ResLife", "")
	AjoutLog("Chargement du fichier settings.ini")
EndFunc;==>LoadConfigsBis
